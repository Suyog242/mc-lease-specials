...
class Extractor < BaseExtractor
  ...
  def load_target_page()
    ...
    @year.each{ |year|
      ...
      .gsub("###year###", year.to_s)
      @finance_url = "https://www.fiatusa.com/hostd/incentives/getoffers.json?zip=#{@zipcode.to_s}&year=#{year}&divisionCode=X&modelCode="
      ...
      response["result"]["data"]["modelYears"].each{ |model_year|
        model_year["models"].each{ |model|
          ...
          model["offers"].each{ |offer|
            ...
            output.mileage_charge =offer["disclaimer"].scan(/.Lessee.*year/).first.strip rescue nil
            output.model_details = offer["vehicles"].first["bodymodelCode"] rescue nil
            output.offer_type = "Lease"
            mixup_title_and_offer = offer["name"] rescue nil
            title2 = mixup_title_and_offer.gsub(/(.*?for).*/, '\1') rescue nil
            output.title2 = title2
            
            offers = mixup_title_and_offer.split('.')
            ...
            output.tax_registration_exclusion =offer["disclaimer"].scan("Tax, title &amp; license extra")[0].gsub("&amp;", "&") rescue nil
            output.offer5 = "Offer expires on #{offer["incentives"][0]["endDate"]}" if !offer["incentives"][0]["endDate"].nil?
            output.disclaimer1 = OpFormat.parse_text(offer["disclaimer"]) rescue nil
            lease_output_data << output            
          }
        }
      }
  ...
  def extract_finance_data()
    ...
    @models.uniq.each{|model_name|
      ...
      if !@finance_response
        output = OpFormat.new  
        output.zip,output.brand,output.offer_type = @zipcode,"FIAT","Finance" 
        ...
        if offer_type["type"].include?("subv_apr") 
          offer_type["offers"].each{|offer|
            ...
            vehicles.each{|model|
              ...
              offer["incentives"].each do |incentive|
                ...
                term = incentive["terms"]
                output.apr_rate = "#{term[0]["value"]} %"  rescue nil
                output.msrp = model["configuredPrice"] rescue nil
                output.offer_start_date = offer["startDate"]
                output.offer_end_date  = offer["endDate"]
          ...
          offer_type["offers"].each do |offer|
            offer["vehicles"].each{|model|
              ...
              output.offer_start_date = offer["startDate"]
              output.offer_end_date = offer["endDate"]
              output.cashback_amount = offer["offerDetails"]["amount"]
              output.msrp = model["configuredPrice"] rescue nil
