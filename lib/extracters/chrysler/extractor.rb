...
class Extractor < BaseExtractor
  ...
  def load_target_page()
    ...
    @year.each{ |year|
      ...
      .gsub("###year###", year.to_s)
      @finance_url = "https://www.chrysler.com/hostd/incentives/getoffers.json?zip=#{@zipcode.to_s}&year=#{year}&divisionCode=C&modelCode="
      ...
      if !@response
        lease_output_data = []
        output = OpFormat.new  
        output.zip,output.brand = @zipcode,"CHRYSLER"                   
          ...
          offer_type["offers"].each{|offer|
            ...
            vehicles.each{|model|
              ...
              offer["incentives"].each do |incentive|
                ...
                output.msrp = model["configuredPrice"]
                ...
                output.disclaimer1 = "#{OpFormat.parse_text(offer["disclaimer"]).gsub(/<("[^"]*"|'[^']*'|[^'">])*>/, '')}" rescue nil
                output.tax_registration_exclusion = model["disclaimer"].match(/Tax.*\./).to_s rescue nil
                finance_output_data << output
                if term.size > 1
