    ...
    models.each do |model|
      begin
        ...
        if(model.xpath(".#{@X['extra_offer_check']}").size == 2)
          ...
          output.disclaimer2 = model.xpath(".#{@X['disclaimer_part1']}")[1].text.strip + model.xpath(".#{@X['disclaimer_part2']}")[1].text.strip
        end
...
#        output.due_at_signing = output.offer2.match(/(\$[\d\,]+) due at signing/)[1]
        output.mileage_charge = output.offer4.match(/(\$[\d\.\s]+\/mile.*?miles)/)[1]
...
#        binding.pry if output.title1 == '2018 CHEVROLET Malibu excludes L'
        if model.xpath(".#{@X['offer1_part1']}").size == 2
         ...
         output.offer2 = model.xpath(".#{@X['offer1_part1']}")[1].text.strip + " " + model.xpath(".#{@X['offer1_part2']}")[2].text.strip
         output.offer3 = model.xpath(".#{@X['offer2']}").inner_html.split('<br>')[1].strip 
         output.offer_end_date = Date.strptime(output.offer3.scan(/Take delivery by ([\d\-]+)/).flatten[0], "%m-%d-%Y").to_s rescue ""
        else
        output.offer1 = model.xpath(".#{@X['offer1_part1']}").text.strip + " " + model.xpath(".#{@X['offer1_part2']}").text.strip
        output.offer2 = model.xpath(".#{@X['offer2']}").inner_html.split('<br>')[1].strip
        output.offer_end_date = Date.strptime(output.offer2.scan(/Take delivery by ([\d\-]+)/).flatten[0], "%m-%d-%Y").to_s rescue ""
        end
        output.apr_rate = output.offer1.scan(/([\d\.\,]+)/).flatten[0]
...
#        binding.pry if output.title1 == '2018 CHEVROLET Malibu excludes L'
        if(model.xpath(".#{@X['extra_offer_check']}").size == 2)
          output.offer1 = model.xpath(".#{@X['title1']}")[1].inner_html.split("<br>")[0].strip
          output.offer2 = model.xpath(".#{@X['title1']}")[2].inner_html.split("<br>")[0].strip
          output.offer3 = model.xpath(".#{@X['offer1_part1']}")[0].text.strip + " " + model.xpath(".#{@X['offer1_part2']}")[0].text.strip
          output.disclaimer1 = model.xpath(".#{@X['offer2']}").inner_html.split('<br>')[1].strip
        else
          output.offer1 = model.xpath(".#{@X['offer1_part1']}")[0].text.strip + " " + model.xpath(".#{@X['offer1_part2']}")[0].text.strip
          output.disclaimer1 = model.xpath(".#{@X['offer2']}").inner_html.split('<br>')[1].strip
        end
    ...
    @logger.debug "Total number of other offers found are #{finance_output_data.size}"
